...
import pandas as pd
# metrics
from sklearn.metrics import accuracy_score, recall_score, precision_score, f1_score, PrecisionRecallDisplay, classification_report
...
def evaluate(args, model, tokenizer, eval_dataset, eval_when_training=False):
    ...
    PrecisionRecallDisplay.from_predictions(y_trues, logits[:, 1], name='Human vs AI')
    ...
    logger.info("***** Classification Report (Eval) *****")
    logger.info("\n" + classification_report(y_trues, y_preds, target_names=target_names, digits=4))
...
def test(args, model, tokenizer, test_dataset, best_threshold=0.5):
    ...
    if len(np.unique(y_trues)) > 1:
        ...
        logger.info("***** Test dataset has no valid labels. Skipping metrics calculation. *****")
...
def generate_result_df(logits, y_trues, y_preds, args):
    df = load_df(args.test_data_file)
    assert len(logits) == len(y_trues) == len(y_preds)
    return pd.DataFrame({"logits": logits, "y_trues": y_trues, "y_preds": y_preds, 
                         "index": list(range(len(logits))), "code": df["code"]})

def write_raw_preds_csv(args, y_preds):
    df = load_df(args.test_data_file)
    df["raw_preds"] = [1 if pred else 0 for pred in y_preds]
    df["raw_preds_label"] = ["AI" if pred else "Human" for pred in y_preds]
    if "label" in df.columns:
        df["label_name"] = ["AI" if val == 1 else "Human" for val in df["label"]]
    elif "target" in df.columns:
        df["label_name"] = ["AI" if val == 1 else "Human" for val in df["target"]]
